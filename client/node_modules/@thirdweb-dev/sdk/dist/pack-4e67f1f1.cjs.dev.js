...
class PackVRF {
  constructor(network, address, storage, options, chainId) {
    let contractWrapper = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : new thirdwebCheckout.ContractWrapper(network, address, IPackVRFAbi__default["default"], options);
    QueryParams._defineProperty(this, "featureName", thirdwebCheckout.FEATURE_PACK_VRF.name);
  ...
  getLinkContract() {
    const linkAddress = thirdwebCheckout.LINK_TOKEN_ADDRESS[this.chainId];
    if (!linkAddress) {
      throw new Error(`No LINK token address found for chainId ${this.chainId}`);
    }
    const contract = new thirdwebCheckout.ContractWrapper(this.contractWrapper.getSignerOrProvider(), linkAddress, ERC20Abi__default["default"], this.contractWrapper.options);
    return new thirdwebCheckout.Erc20(contract, this.storage, this.chainId);
  }
}
...
class Pack extends thirdwebCheckout.StandardErc1155 {
   ...
   */
  get vrf() {
    return thirdwebCheckout.assertEnabled(this._vrf, thirdwebCheckout.FEATURE_PACK_VRF);
  }
  ...
  detectVrf() {
    if (thirdwebCheckout.detectContractFeature(this.contractWrapper, "PackVRF")) {
