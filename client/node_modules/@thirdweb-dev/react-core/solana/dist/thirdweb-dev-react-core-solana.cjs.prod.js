...
var requiredParam = require('../../dist/required-param-3dc80806.cjs.prod.js');
var auth = require('@thirdweb-dev/auth');
var solana = require('@thirdweb-dev/auth/solana');
...
const ThirdwebAuthProvider = _ref => {
  ...
  const authContext = react.useMemo(() => {
    ...
    if (identity) {
      context.auth = new auth.ThirdwebAuth(new solana.SignerWallet(identity), value.domain);
    }
    return context;
  }, [value]);
...
const ThirdwebSDKProvider = _ref => {
  ...
  react.useEffect(() => {
    if (network) {
      const _sdk = solana$1.ThirdwebSDK.fromNetwork(network);
