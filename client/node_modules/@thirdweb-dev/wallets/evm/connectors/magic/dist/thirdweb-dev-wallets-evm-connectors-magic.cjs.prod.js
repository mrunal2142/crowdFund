...
class MagicConnector extends core.Connector {
  constructor(config) {
    super(config);
    ...
    try {
      ...
      if (isAuthenticated) {
        return {
          ...
          account: utils_js.getAddress(await this.getAccount())
        };
      }
      ...
      if (!this.isModalOpen) {
        ...
        const account = await signer.getAddress();
        return {
          account: utils_js.getAddress(account),
          chain: {
            id: chainId_,
            unsupported: false
          },
      ...
      throw new core.UserRejectedRequestError("User rejected request");
    } catch (error) {
      throw new core.UserRejectedRequestError("Something went wrong");
    }
  }
  async getChainId() {
    ...
    if (typeof networkOptions === "object") {
      ...
      if (chainID) {
        return core.normalizeChainId(chainID);
      }
    }
