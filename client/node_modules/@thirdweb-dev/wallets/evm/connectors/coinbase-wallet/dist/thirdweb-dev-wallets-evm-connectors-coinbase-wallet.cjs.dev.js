...
var classPrivateMethodGet = require('../../../../dist/classPrivateMethodGet-04ef05fc.cjs.dev.js');
...
class CoinbaseWalletConnector extends core.Connector {
  constructor(_ref) {
    let {
      chains,
      options
  ...
  async getProvider() {
    if (!classPrivateFieldGet._classPrivateFieldGet(this, _provider)) {
      ...
      classPrivateFieldGet._classPrivateFieldSet(this, _client, new CoinbaseWalletSDK(this.options));
      const walletExtensionChainId = classPrivateFieldGet._classPrivateFieldGet(this, _client).walletExtension?.getChainId();
      const chain = this.chains.find(chain_ => this.options.chainId ? chain_.id === this.options.chainId : chain_.id === walletExtensionChainId) || this.chains[0];
      const chainId = this.options.chainId || chain?.id;
      const jsonRpcUrl = this.options.jsonRpcUrl || chain?.rpcUrls.default.http[0];
      classPrivateFieldGet._classPrivateFieldSet(this, _provider, classPrivateFieldGet._classPrivateFieldGet(this, _client).makeWeb3Provider(jsonRpcUrl, chainId));
    }
  ...
  async switchChain(chainId) {
    ...
    try {
      ...
      if (!chain) {
        throw new core.ChainNotConfiguredError({
          chainId,
          connectorId: this.id
        });
      }
      ...
      if (error.code === 4902) {
        try {
          ...
          if (classPrivateMethodGet._classPrivateMethodGet(this, _isUserRejectedRequestError, _isUserRejectedRequestError2).call(this, addError)) {
            throw new core.UserRejectedRequestError(addError);
          }
          throw new core.AddChainError();
        }
      }
