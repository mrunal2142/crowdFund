...
class InjectedConnector extends core.Connector {
  constructor() {
    let {
      chains,
      options: options_
    ...
    classPrivateFieldGet._classPrivateFieldInitSpec(this, _switchingChains, {
      writable: true,
      value: void 0
    });
    ...
    defineProperty._defineProperty(this, "onDisconnect", async () => {
      ...
      if (this.options.shimChainChangedDisconnect && classPrivateFieldGet._classPrivateFieldGet(this, _switchingChains)) {
        classPrivateFieldGet._classPrivateFieldSet(this, _switchingChains, false);
        return;
      }
      this.emit("disconnect");
      // Remove shim signalling wallet is disconnected
      if (this.options.shimDisconnect) {
        await storage.getConnectorStorage().removeItem(this.shimDisconnectKey);
      }
    });
  ...
  async getChainId() {
    ...
    return provider.request({
      method: "eth_chainId"
    }).then(core.normalizeChainId);
  }
  async getProvider() {
    const provider = this.options.getProvider();
    if (provider) {
      classPrivateFieldGet._classPrivateFieldSet(this, _provider, provider);
    }
  ...
  async isAuthorized() {
    try {
      ...
      !(await storage.getConnectorStorage()?.getItem(this.shimDisconnectKey))) {
        return false;
      }
      const provider = await this.getProvider();
      if (!provider) {
  ...
  async switchChain(chainId) {
    if (this.options.shimChainChangedDisconnect) {
      classPrivateFieldGet._classPrivateFieldSet(this, _switchingChains, true);
    }
    const provider = await this.getProvider();
    if (!provider) {
      throw new core.ConnectorNotFoundError();
